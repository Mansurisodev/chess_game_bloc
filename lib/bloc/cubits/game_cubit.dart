...
class GameCubit extends Cubit<GameState> {
  ...
  factory GameCubit.initial() {
    ...
    return GameCubit(GameState(activeColor: GameColors.white, selectedCell: null, board: board, isAIthinking: false, availablePositionsHash: {}));
  }
  ...
  void selectCell(Cell? newCell) {
    emit(state.copyWith(selectedCell: newCell, availablePositionsHash: state.board.getAvailablePositionsHash(newCell)));
  }
  ...
  void moveFigure(Cell toCell) async {
    ...
    emit(state.copyWith(board: state.board.copyThis(), activeColor: state.activeColor.getOpposite()));
